import { useState } from "react";
import { Github, ExternalLink, ArrowBigDown, ChevronLeft, ChevronRight } from "lucide-react";
import { cn } from "../../lib/utils";
import MostRecentProjectPic from "@/assets/IsingWebapp.gif"; 
import Project1 from "@/assets/XYWebapp.gif";
import Project2 from "@/assets/Project2.png";
import Project3 from "@/assets/Project3.png";


const featuredProject = {
  title: "Ising Webapp",
  period: "2025 • In progress",
  description: (
    <>
      An extension of my previous project,{" "}
      <a
        href="https://github.com/swestastic/Ising_GUI"
        target="_blank"
        rel="noopener noreferrer"
        className="underline text-primary"
      >
        Ising_GUI
      </a>
      , this web application allows users to simulate the 2D Ising model using various algorithms including Metropolis, Glauber, Heatbath, Wolff, Swendsen-Wang, and Kawasaki. The app is built in Rust and compiled to WebAssembly for high performance, and features an intuitive interface for adjusting parameters and visualizing results in real-time. It also supports simulated annealing across temperatures and exporting data for further analysis.
    </>
  ),
  tags: ["Rust", "WebAssembly", "JavaScript", "HTML", "CSS"],
  images: [MostRecentProjectPic], // Add more images to the array
  demoUrl: "https://swestastic.github.io/Ising_Rust/", 
  githubUrl: "https://github.com/swestastic/Ising_Rust", 
};

const projects = [
  {
    title: "XY Webapp",
    description: (
      <>
        An extension of my other projects{" "}
        <a
          href="https://github.com/swestastic/XY_GUI"
          target="_blank"
          rel="noopener noreferrer"
          className="underline text-primary"
        >
          XY_GUI
        </a>{" "}
        and{" "}
        <a
          href="https://swestastic.github.io/Ising_Rust/"
          target="_blank"
          rel="noopener noreferrer"
          className="underline text-primary"
        >
          Ising Webapp
        </a>
        , this web application allows users to simulate the 2D XY model using various algorithms including Metropolis, Wolff, and Overrelaxation. The app is built in Rust and compiled to WebAssembly for high performance, and features an intuitive interface for adjusting parameters and visualizing results in real-time.
      </>
    ),
    images: [Project1], // Add more images to the array
    tags: ["Rust", "WebAssembly", "JavaScript", "HTML", "CSS"],
    demoUrl: "https://swestastic.github.io/XY_Rust/",
    githubUrl: "https://github.com/swestastic/XY_Rust",
  },
  {
    title: "PiConsult",
    description: (
      <>
        This project uses Python to record data over serial on a Raspberry Pi and then display it on a small SPI display. Great for monitoring engine data that is stored in the ECU, such as engine RPM and coolant temperature. The logic is the same as my desktop app,{" "}
        <a
          href="https://github.com/swestastic/PyConsult"
          target="_blank"
          rel="noopener noreferrer"
          className="underline text-primary"
        >
          PyConsult
        </a>
        , but now it comes in a small form factor which you can leave in your car!
      </>
    ),
    images: [Project2], // Add more images to the array
    tags: ["Raspberry Pi", "Python", "RS232 Serial", "SPI", "OLED Displays"],
    demoUrl: "#",
    githubUrl: "https://github.com/swestastic/PiConsult",
  },
  {
    title: "SmoQyDQMC.jl",
    description: (
      <>
        A fork of the determinant quantum Monte Carlo (DQMC) package SmoQyDQMC.jl
        which aims to tackle the sometimes large amounts of file I/O generated by the original package. Great for doing many DQMC runs and significantly decreasing the amount of data written to disk, while maintaining accuracy and original functionality.
      </>
    ),
    images: [Project3], // Add more images to the array
    tags: ["Julia", "File I/O", "Quantum Monte Carlo"],
    demoUrl: "#",
    githubUrl: "https://github.com/swestastic/SmoQyDQMC.jl",
  },
];


export default function MyWorkSection() {
  const p = featuredProject;
  
  const [featuredImageIndex, setFeaturedImageIndex] = useState(0);
  const [currentImageIndex, setCurrentImageIndex] = useState(
    projects.reduce((acc, _, index) => {
      acc[index] = 0;
      return acc;
    }, {})
  );
  const [hoveredCard, setHoveredCard] = useState(null);
  const [hoveredFeatured, setHoveredFeatured] = useState(false);

  const handlePrevImage = (projectIndex, e) => {
    e.preventDefault();
    e.stopPropagation();
    setCurrentImageIndex(prev => ({
      ...prev,
      [projectIndex]: prev[projectIndex] === 0 
        ? projects[projectIndex].images.length - 1 
        : prev[projectIndex] - 1
    }));
  };

  const handleNextImage = (projectIndex, e) => {
    e.preventDefault();
    e.stopPropagation();
    setCurrentImageIndex(prev => ({
      ...prev,
      [projectIndex]: prev[projectIndex] === projects[projectIndex].images.length - 1 
        ? 0 
        : prev[projectIndex] + 1
    }));
  };

  const handlePrevFeaturedImage = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setFeaturedImageIndex(prev => 
      prev === 0 ? p.images.length - 1 : prev - 1
    );
  };

  const handleNextFeaturedImage = (e) => {
    e.preventDefault();
    e.stopPropagation();
    setFeaturedImageIndex(prev => 
      prev === p.images.length - 1 ? 0 : prev + 1
    );
  };

  return (
    <section id="projects" className={cn("relative py-2 md:py-2 bg-background text-foreground")}>
      <div className="container">
        {/* Header */}
        <div className="mx-auto max-w-3xl text-center mb-10 md:mb-12 animate-[var(--animate-fade-in)]">
          {/* <div className="inline-flex items-center gap-2 rounded-full border border-border/60 px-3 py-1 text-xs md:text-sm text-foreground/70">
            <span className="size-1.5 rounded-full bg-primary/70" />
            My Work
          </div> */}
          <h2 className="mt-4 text-2xl md:text-3xl font-bold tracking-tight">Most recent project</h2>
          <p className="mt-3 text-foreground/70">A quick look at what I've been building lately.</p>
        </div>

        {/* Featured card */}
        <article className="rounded-2xl bg-card/70 border border-border shadow-sm overflow-hidden group">
          <div className="grid grid-cols-1 md:grid-cols-2">
            {/* Image side */}
            <div 
              className="relative"
              onMouseEnter={() => setHoveredFeatured(true)}
              onMouseLeave={() => setHoveredFeatured(false)}
            >
              <div className="gradient-border p-[1px] h-full md:rounded-l-2xl">
                <div className="h-full w-full bg-card/80 md:rounded-l-2xl overflow-hidden">
                  <img
                    src={p.images[featuredImageIndex]}
                    alt={`${p.title} IsingWebapp screenshot`}
                    loading="lazy"
                    className="w-full h-full object-cover md:aspect-[16/11] transition-transform duration-300 group-hover:scale-[1.02]"
                  />
                  {p.images.length > 1 && (
                    <div 
                      className="absolute inset-0 z-30 transition-opacity duration-300"
                      style={{ opacity: hoveredFeatured ? 1 : 0.15 }}
                    >
                      <button
                        onClick={handlePrevFeaturedImage}
                        className="absolute left-4 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-100 text-black rounded-full p-3 shadow-xl border-2 border-gray-200"
                        aria-label="Previous image"
                      >
                        <ChevronLeft className="size-5" />
                      </button>
                      <button
                        onClick={handleNextFeaturedImage}
                        className="absolute right-4 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-100 text-black rounded-full p-3 shadow-xl border-2 border-gray-200"
                        aria-label="Next image"
                      >
                        <ChevronRight className="size-5" />
                      </button>
                      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 bg-black/30 px-3 py-2 rounded-full backdrop-blur-sm">
                        {p.images.map((_, imgIndex) => (
                          <div
                            key={imgIndex}
                            className={`h-2 w-2 rounded-full transition-all ${
                              imgIndex === featuredImageIndex
                                ? 'bg-white w-6'
                                : 'bg-white/60'
                            }`}
                          />
                        ))}
                      </div>
                    </div>
                  )}
                </div>
              </div>
              <span className="absolute top-3 left-3 inline-flex items-center gap-2 rounded-full bg-background/80 backdrop-blur px-3 py-1 text-xs border border-border/60">
                <span className="size-1.5 rounded-full bg-primary" />
                {p.period}
              </span>
            </div>

            {/* Content side */}
            <div className="p-6 md:p-8 flex flex-col justify-center animate-[var(--animate-fade-in-delay-1)]">
              <h3 className="text-xl md:text-2xl font-semibold">{p.title}</h3>
              <p className="mt-3 text-foreground/70 leading-relaxed">{p.description}</p>

              <div className="mt-4 flex flex-wrap gap-2">
                {p.tags.map((t) => (
                  <span
                    key={t}
                    className="text-xs md:text-sm rounded-full border border-border/60 bg-background/60 px-3 py-1 text-foreground/80"
                  >
                    {t}
                  </span>
                ))}
              </div>

              <div className="mt-6 flex flex-wrap gap-3">
                {p.demoUrl && p.demoUrl !== "#" && (
                  <a
                    href={p.demoUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 rounded-xl bg-primary text-primary-foreground px-4 py-2 transition-transform duration-200 hover:scale-[1.02]"
                  >
                    <ExternalLink className="size-4" />
                    Live Demo
                  </a>
                )}
                {p.githubUrl && (
                  <a
                    href={p.githubUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2 hover:bg-background/80"
                  >
                    <Github className="size-4" />
                    GitHub
                  </a>
                )}
              </div>
            </div>
          </div>
        </article>
      </div>
      <p className="text-center mb-6 max-w-2xl mx-auto text-xl mt-18">
       These are a few of my favorite projects that I have built. More available under the simulations tab and on my GitHub.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {projects.map((project, index) => (
                <div 
                  key={index} 
                  className="bg-card overflow-hidden rounded-lg shadow-xs card-hover group"
                  onMouseEnter={() => setHoveredCard(index)}
                  onMouseLeave={() => setHoveredCard(null)}
                >
                    <div className="relative aspect-video bg-muted overflow-hidden">
                        <img 
                          src={project.images[currentImageIndex[index]]} 
                          alt={project.title} 
                          className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110"
                        />
                        {project.images.length > 1 && (
                          <div 
                            className="absolute inset-0 z-30 transition-opacity duration-300"
                            style={{ opacity: hoveredCard === index ? 1 : 0.15 }}
                          >
                            <button
                              onClick={(e) => handlePrevImage(index, e)}
                              className="absolute left-4 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-100 text-black rounded-full p-3 shadow-xl border-2 border-gray-200"
                              aria-label="Previous image"
                            >
                              <ChevronLeft className="size-5" />
                            </button>
                            <button
                              onClick={(e) => handleNextImage(index, e)}
                              className="absolute right-4 top-1/2 -translate-y-1/2 bg-white hover:bg-gray-100 text-black rounded-full p-3 shadow-xl border-2 border-gray-200"
                              aria-label="Next image"
                            >
                              <ChevronRight className="size-5" />
                            </button>
                            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2 bg-black/30 px-3 py-2 rounded-full backdrop-blur-sm">
                              {project.images.map((_, imgIndex) => (
                                <div
                                  key={imgIndex}
                                  className={`h-2 w-2 rounded-full transition-all ${
                                    imgIndex === currentImageIndex[index]
                                      ? 'bg-white w-6'
                                      : 'bg-white/60'
                                  }`}
                                />
                              ))}
                            </div>
                          </div>
                        )}
                    </div>

                    <div className="p-6">

                        <div className="flex flex-wrap gap-2 mb-4">
                            {project.tags.map((tag, key) => (
                                
                                <span 
                                key={key}
                                className="px-2 py-1 text-xs border font-medium rounded-full bg-secondary text-secondary-foreground">
                                    {tag}
                                </span>
                            ))}
                        </div>

                    
                        <h3 className="text-xl font-semibold mb-1">{project.title}</h3>
                        <p className="text-sm mb-4">{project.description}</p>
                        <div className="flex justify-between items-center">
                            <div className="flex space-x-3">
                                {project.demoUrl && project.demoUrl !== "#" && (
                                    <a 
                                        href={project.demoUrl} 
                                        target="_blank"
                                        rel="noreferrer"
                                        className="inline-flex items-center gap-2 rounded-xl bg-primary text-primary-foreground px-4 py-2 transition-transform duration-200 hover:scale-[1.02]"
                                    >
                                        <ExternalLink className="size-4"/>Live Demo
                                    </a>
                                )}
                                {project.githubUrl && project.githubUrl !== "#" && (
                                    <a href={project.githubUrl} 
                                    target="_blank"
                                    rel="noreferrer"
                                    className="inline-flex items-center gap-2 rounded-xl border border-border px-4 py-2 hover:bg-background/80"
                                    >
                                        <Github className="size-4"/>GitHub
                                    </a>
                                )}
                            </div>
                        </div>
                        </div>
                </div>
            ))}
        </div>
    </section>
  );
}